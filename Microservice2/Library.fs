...
module Main=
  ...
  let defaultApp1Microservice2Config =Seq.empty

  let getMicroservice2Configuration (args:string[])=Seq.empty
  let getAppConfig (args:string[])=Seq.empty

  let getConfig:GetConfigurationFunction =
    (fun (args:string[])->defaultApp1Microservice2Config)

  let getIncomingStream:GetIncomingStreamFunction=
    (fun (appConfig)->defaultApp1Microservice2Config,emptyAppDataTransfer)
